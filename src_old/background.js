...
chrome.runtime.onInstalled.addListener(() => {
  // chrome.storage.local.set({ focusDuration: 5, breakDuration: 3 });  // USE IN TEST
  chrome.storage.local.set({ focusDuration: 25*60, breakDuration: 5*60 });  // USE IN PROD
});
...
function playBell() {
  const audio = new Audio(chrome.runtime.getURL('sounds/ding-small-bell.mp3'));
  audio.play();
}
